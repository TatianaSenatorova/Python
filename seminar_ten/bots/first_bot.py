...
GENDER, HELLO, PICTURE, PHOTO, LOCATION, BIO = range(6)
...
def send_picture(update, _):
    update.message.reply_text(
        'Напиши мне /картинка и я пришлю ее тебе'
    )
    img = open('free-sticker-hello-8136247.png', 'rb')
...
def gender(update, _):
    ...
    logger.info("Приветствие %s: %s", user.first_name, update.message.text)
...
if __name__ == '__main__':
    ...
    conv_handler = ConversationHandler( # здесь строится логика разговора
        ...
        states={
            ...
            HELLO: [MessageHandler(Filters.regex('^(hello|привет)$'), hello)],
            PICTURE: [MessageHandler(Filters.text('картинка'), send_picture)],
